...
void floatTetWild::simplify(std::vector<Vector3>& input_vertices, std::vector<Vector3i>& input_faces, std::vector<int>& input_tags,
    ...
    // NOTE: uint8_t, not vector<bool>: the TBB collapsing/swapping passes
    // write these flags from multiple threads at arbitrary indices. The
    // one-ring safe sets make the INDICES disjoint, but vector<bool> packs
    // 8 flags per byte, so writes to different indices still race on the
    // shared word (lost updates -> nondeterministic simplification and
    // sporadic crashes). Byte-sized flags are distinct memory locations.
    std::vector<uint8_t> v_is_removed(input_vertices.size(), false);
    std::vector<uint8_t> f_is_removed(input_faces.size(), false);
...
void floatTetWild::collapsing(std::vector<Vector3>& input_vertices, std::vector<Vector3i>& input_faces,
        ...
        std::vector<uint8_t>& v_is_removed, std::vector<uint8_t>& f_is_removed, std::vector<std::unordered_set<int>>& conn_fs){
...
void floatTetWild::check_surface(std::vector<Vector3>& input_vertices, std::vector<Vector3i>& input_faces, const std::vector<uint8_t>& f_is_removed,
                   const AABBWrapper& tree, const Parameters& params) {
